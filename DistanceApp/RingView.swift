...
//  DistanceApp
//
//  Created by Adrian Henry on 16/04/2020.
...
struct RingView: View {
    ...
    var body: some View {
        let gradient = AngularGradient(gradient: Gradient(colors: [startColor, endColor]),
                                       center: .center, startAngle: Angle(degrees: 0),
...
struct RingShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        var path = Path()
        path.addArc(center: center, radius: radius, startAngle: Angle(degrees: 0),
                    endAngle: Angle(degrees: 360 * currentPercentage), clockwise: false)
